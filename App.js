...
const url =
  "mongodb+srv://Vishesh:Vishesh@cluster0.evghetg.mongodb.net/?retryWrites=true&w=majority";
const config = {
...
// Details of GoogleDrive---------------------------------------------------------- old drive
...
// const REFRESH_TOKET ="1//04BMM26PsOtxHCgYIARAAGAQSNwF-L9Ir-rziVaJllZBmQxSP7MZLFwpSXuQqVZi-vS0vuQyTPcM8F-FvkllvazcnZwUC6X-ar7k";


// Details of GoogleDrive---------------------------------------------------------- new drive 
const CLIENT_ID ="324975538567-rh14q555rgauuh7colpaginrk62uhctu.apps.googleusercontent.com";
const CLIENT_SECRET = "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";
const REDIRECT_URL = "https://developers.google.com/oauthplayground";

const REFRESH_TOKET ="1//04HrddfVFW86FCgYIARAAGAQSNwF-L9Irhin2hqK7Q4kBhddvD-BIUKZpECb5R3EGjAZbNSjmkyCtEtT4-P7QgGADTJBQgsy2_v8";
    ...
    try {
      ...
      if (Dates.length < 1) {
        ...
        if (udate > Dates[0].LatestDate) {
          await client
            .db("test")
            .collection("googledates")
            .updateOne({ name: "yes" }, { $set: { LatestDate: udate } });
        } else {
          await client
            .db("test")
            .collection("googledates")
            .updateOne(
              { name: "yes" },
              { $set: { LatestDate: Dates[0].LatestDate } }
            );
        }
        ...
        if (udate < Dates[0].StartDate) {
          await client
            .db("test")
            .collection("googledates")
            .updateOne({ name: "yes" }, { $set: { StartDate: udate } });
        }
      }
    } catch (err) {
      console.log(err.message);
    ...
    fs.rm(path.join(__dirname, "uploads"), { recursive: true }, () => {
      ...
      fs.mkdir(path.join(__dirname, "uploads"), { recursive: true }, (err) => {
        if (err) {
          ...
          fs.writeFile(
            path.join(__dirname, "uploads/dammy.txt"),
            "this is dammy file",
            function (err) {
              if (err) throw err;
...
app.delete("/Delete/:id", async (req, res) => {
  ...
  async function Mongodelete(id) {
    try {
      ...
      if (allData.deletedCount) res.send("deleted");
      else res.send("please enter correct id");
    } catch (err) {
      console.log(err);
    }
  }
